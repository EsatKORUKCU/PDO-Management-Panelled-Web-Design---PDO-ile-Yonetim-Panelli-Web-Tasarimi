...
if ($_POST){
    ...
    */
    ...
    $sorgu = $baglan-> prepare("update bolum3 set baslik=?,icerik=?,resim=? where (id=?)");
    $guncelle = $sorgu->execute(array($baslik,$icerik,$resim,$id));
    if ($guncelle){
        echo "<script>
            alert ('Kayıt Düzenlendi!');
            window.location.href ='bolum3.php';
            </script>";
    }
}

// Verilerin Düzenleme İçin Çekilmesi
$sorgu = $baglan->query("select * from bolum3")->fetch(PDO::FETCH_ASSOC);
?>
